...
// api: non-existent merchant must map to 404, invalid path ID to 400.
func (s *MerchantApiTestSuite) TestMerchantApiNotFound() {
	apigatewaymiddlewares.RegisterErrorHandler(s.echo)

	req := httptest.NewRequest(http.MethodGet, "/api/merchant-query/999999", nil)
	rec := httptest.NewRecorder()
	s.echo.ServeHTTP(rec, req)
	s.Equal(http.StatusNotFound, rec.Code, "non-existent merchant must be 404, got %d: %s", rec.Code, rec.Body.String())
}

func (s *MerchantApiTestSuite) TestMerchantApiInvalidID() {
	apigatewaymiddlewares.RegisterErrorHandler(s.echo)

	req := httptest.NewRequest(http.MethodGet, "/api/merchant-query/abc", nil)
	rec := httptest.NewRecorder()
	s.echo.ServeHTTP(rec, req)
	s.Equal(http.StatusBadRequest, rec.Code, "invalid merchant ID must be 400, got %d: %s", rec.Code, rec.Body.String())
}
