...
import (
	...
	"github.com/MamangRust/monolith-ecommerce-grpc-banner/internal/middleware"
...
func init() {
	port = viper.GetInt("GRPC_BANNER_ADDR")
	if port == 0 {
		port = 50064
	}
...
func NewServer(ctx context.Context) (*Server, func(context.Context) error, error) {
	...
	shutdownTracerProvider, err := otel_pkg.InitTracerProvider("Banner-service", ctx)
	...
	myredis := redis.NewClient(&redis.Options{
		...
		DB:           viper.GetInt("REDIS_DB_BANNER"),
...
func (s *Server) Run() {
	...
	metricsAddr := fmt.Sprintf(":%s", viper.GetString("METRIC_BANNER_ADDR"))
	...
	go func() {
		defer wg.Done()
		s.Logger.Info("Metrics server listening on :8094")
	...
	go func() {
		defer wg.Done()
		s.Logger.Info("gRPC server listening on :50064")
