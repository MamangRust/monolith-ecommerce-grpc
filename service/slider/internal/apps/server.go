...
import (
	...
	"github.com/MamangRust/monolith-ecommerce-grpc-slider/internal/middleware"
...
func init() {
	port = viper.GetInt("GRPC_SLIDER_ADDR")
	if port == 0 {
		port = 50062
	}
...
func NewServer(ctx context.Context) (*Server, func(context.Context) error, error) {
	logger, err := logger.NewLogger("slider-service")
	if err != nil {
	...
	shutdownTracerProvider, err := otel_pkg.InitTracerProvider("Slider-service", ctx)
	...
	myredis := redis.NewClient(&redis.Options{
		...
		DB:           viper.GetInt("REDIS_DB_SLIDER"),
...
func (s *Server) Run() {
	...
	metricsAddr := fmt.Sprintf(":%s", viper.GetString("METRIC_SLIDER_ADDR"))
	...
	go func() {
		defer wg.Done()
		s.Logger.Info("Metrics server listening on :8092")
	...
	go func() {
		defer wg.Done()
		s.Logger.Info("gRPC server listening on :50062")
