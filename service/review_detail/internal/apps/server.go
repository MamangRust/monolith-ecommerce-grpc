...
import (
	...
	"github.com/MamangRust/monolith-ecommerce-grpc-review-detail/internal/middleware"
...
func init() {
	port = viper.GetInt("GRPC_REVIEW_DETAIL_ADDR")
	if port == 0 {
		port = 50069
	}
...
func NewServer(ctx context.Context) (*Server, func(context.Context) error, error) {
	logger, err := logger.NewLogger("review-detail-service")
	if err != nil {
	...
	shutdownTracerProvider, err := otel_pkg.InitTracerProvider("ReviewDetail-service", ctx)
	...
	myredis := redis.NewClient(&redis.Options{
		...
		DB:           viper.GetInt("REDIS_DB_REVIEW_DETAIL"),
...
func (s *Server) Run() {
	...
	metricsAddr := fmt.Sprintf(":%s", viper.GetString("METRIC_REVIEW_DETAIL_ADDR"))
	...
	go func() {
		defer wg.Done()
		s.Logger.Info("Metrics server listening on :8099")
	...
	go func() {
		defer wg.Done()
		s.Logger.Info("gRPC server listening on :50069")
