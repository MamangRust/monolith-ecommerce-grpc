...
func init() {
	port = viper.GetInt("GRPC_MERCHANT_DETAIL_ADDR")
	if port == 0 {
		port = 50067
	}
...
func NewServer(ctx context.Context) (*Server, func(context.Context) error, error) {
	logger, err := logger.NewLogger("merchant-detail-service")
	...
	myredis := redis.NewClient(&redis.Options{
		...
		DB:           viper.GetInt("REDIS_DB_MERCHANT_DETAIL"),
...
func (s *Server) Run() {
	...
	metricsAddr := fmt.Sprintf(":%s", viper.GetString("METRIC_MERCHANT_DETAIL_ADDR"))
	...
	pb.RegisterMerchantSocialServiceServer(grpcServer, s.Handlers.MerchantSocialLink)
	...
	go func() {
		defer wg.Done()
		s.Logger.Info("Metrics server listening on :8097")
	...
	go func() {
		defer wg.Done()
		s.Logger.Info("gRPC server listening on :50067")
