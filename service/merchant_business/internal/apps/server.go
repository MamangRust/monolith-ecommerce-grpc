...
import (
	...
	"github.com/MamangRust/monolith-ecommerce-grpc-merchant_business/internal/handler"
	"github.com/MamangRust/monolith-ecommerce-grpc-merchant_business/internal/middleware"
...
func init() {
	port = viper.GetInt("GRPC_MERCHANT_BUSINESS_ADDR")
	if port == 0 {
		port = 50066
	}
...
func NewServer(ctx context.Context) (*Server, func(context.Context) error, error) {
	logger, err := logger.NewLogger("merchant-business-service")
	...
	shutdownTracerProvider, err := otel_pkg.InitTracerProvider("MerchantBusiness-service", ctx)
	...
	myredis := redis.NewClient(&redis.Options{
		...
		DB:           viper.GetInt("REDIS_DB_MERCHANT_BUSINESS"),
...
func (s *Server) Run() {
	...
	metricsAddr := fmt.Sprintf(":%s", viper.GetString("METRIC_MERCHANT_BUSINESS_ADDR"))
	...
	go func() {
		defer wg.Done()
		s.Logger.Info("gRPC server listening on :50066")
