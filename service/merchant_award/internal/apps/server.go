...
import (
	...
	"github.com/MamangRust/monolith-ecommerce-grpc-merchant_award/internal/middleware"
...
func init() {
	port = viper.GetInt("GRPC_MERCHANT_AWARD_ADDR")
	if port == 0 {
		port = 50065
	}
...
func NewServer(ctx context.Context) (*Server, func(context.Context) error, error) {
	logger, err := logger.NewLogger("merchant-award-service")
	...
	shutdownTracerProvider, err := otel_pkg.InitTracerProvider("Cart-service", ctx)
	...
	myredis := redis.NewClient(&redis.Options{
		...
		DB:           viper.GetInt("REDIS_DB_MERCHANT_AWARD"),
...
func (s *Server) Run() {
	...
	go func() {
		defer wg.Done()
		s.Logger.Info("Metrics server listening on :8095")
	...
	go func() {
		defer wg.Done()
		s.Logger.Info("gRPC server listening on :50065")
